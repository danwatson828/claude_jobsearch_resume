...
All text generated for cover letters or resume summaries must be passed through `humanize()` in `tools/generate_batch_content.py` before being saved or uploaded.

The humanizer strips known AI-tell phrases ("exciting opportunity", "passionate", "deeply motivating", "player-coach", etc.) and ensures the writing sounds like a person, not a language model.

This applies to:
- Cover letters generated by `build_cover_letter()`
- Any text written to `.tmp/cover_*.txt` files
- Any resume summary text placed on line 4

Do not skip the humanizer step even if the text looks clean — run it as a final pass before writing to batch_content.json.
