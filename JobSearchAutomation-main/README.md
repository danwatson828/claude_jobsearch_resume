...
| Resume bullet library | Free | Edit `resume/bullets.md` with your own experience, organized by role. Claude selects from this library when tailoring each resume. | Low |
...
- Tailors your resume for each new job by selecting and adapting bullets from `resume/bullets.md`
...
You can add any job from any source — company careers pages, referrals, job boards other
than LinkedIn — using the Manual Jobs tab in your Google Sheet.

1. Open your Google Sheet tracker and go to the **Manual Jobs** tab
   (Claude creates it automatically on first use)
2. Add a row with: Company, Job Title, and the job posting URL
3. Tell Claude:
...
Claude will fetch each job description, tailor your resume, write a cover letter, and
write the Drive links back into the Manual Jobs tab.

Rows marked Done or Error are never re-processed. To retry a failed row, clear the
Status cell and re-run.
...
resume/          -> resume/bullets.md — your curated bullet library (source of truth for tailoring)
...
To update your resume content, edit `resume/bullets.md` directly — add new accomplishments, adjust numbers, or retag bullets. Changes take effect on the next run.
...
| Want to update your resume content | Edit `resume/bullets.md` — add, remove, or retag bullets. No `.env` change needed. |
